...
void ode_mode_sim::update(rp_box b) {
  ...
  void ode_mode_sim::update_box(rp_box b, DVector v, double time) {
    ...
    for (int i = 0; i < v.size(); i++){
      ...
      rp_binf(rp_box_elem(b, rp_id)) = v[i];
      rp_bsup(rp_box_elem(b, rp_id)) = v[i];
    }

    // assign parameters at time t equal to time 0
    for (int i = 0; i < m_0_pars.size(); i++){
      ...
      rp_binf(rp_box_elem(b, rp_id_t)) = rp_binf(rp_box_elem(b, rp_id_0));
      rp_bsup(rp_box_elem(b, rp_id_t)) = rp_bsup(rp_box_elem(b, rp_id_0));

      DREAL_LOG_DEBUG << "ode_mode_sim::update_box set "
                      << _t_var << " = "
                      << _0_var << " = "
                      << "[" << rp_binf(rp_box_elem(b, rp_id_0))
                      << ", " << rp_bsup(rp_box_elem(b, rp_id_0))
                      << "]";
    }

    // set time variable
     int const rp_id = m_enode_to_rp_id[m_time];
     rp_binf(rp_box_elem(b, rp_id)) = time;
     rp_bsup(rp_box_elem(b, rp_id)) = time;
     DREAL_LOG_DEBUG << "ode_mode_sim::update_box set "
                      << m_time << " = "
...
DVector ode_mode_sim::varlist_to_DVector(vector<Enode*> const & vars) {
  DVector intvs (vars.size());
    ...
    for (unsigned i = 0; i < vars.size(); i++) {
        ...
        double lb = get_lb(var);
        double ub = get_ub(var);
        val = lb;// interval(lb, ub);
        DREAL_LOG_INFO << "ode_solver::varlist_to_DVector: The value of " << var->getCar()->getName() << ": " << val;
    }
    return intvs;
}
...
//  return true if it violates invariant otherwise return false.
  bool ode_mode_sim::inner_loop_forward(DTaylor & solver, interval const & prevTime,
    ...
    DREAL_LOG_DEBUG << "ode_mode_sim::inner_loop_forward " << m_T;
    interval const stepMade = solver.getStep();
    ...
    for (interval subsetOfDomain : intvs) {
      DREAL_LOG_DEBUG << "ode_mode_sim::inner_loop_forward: " << subsetOfDomain;
        interval dt = prevTime + subsetOfDomain;
        DVector v = curve(subsetOfDomain.rightBound());
        if (!true // check_invariant(v, m_inv)
            ) {
            // TODO(soonhok): invariant
            return true;
        }
